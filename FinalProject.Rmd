...
## Dataset
  - updated 11 days ago (popularity score)
  - https://towardsdatascience.com/what-makes-a-song-likeable-dbfdb7abe404 explains variables, explain them and what they mean
  - last 50 years (1970-2020)

## Questions
  - what factors contribute to making a song popular (>70) or not
  - does explicit content songs have other similar factors
    helps with families and parents 
  - do certain factors make a song more or less valence (>=.6 happy, .4<x<.6, <=.4 sad/angry)
...
## Purpose for Exploration
  ...
  - PREVIOUS RESEARCH/EVIDENCE HERE

## Methods being Used 
  -decision trees bc more parameters, both qualitative and quantitative
  -other reasons(?)

```{r, echo=FALSE, cache=TRUE}
spotify = read.csv("spotify_data_1921_2020.csv")
```
...
#make binary popularity variable (>=70 is 1, 0 otherwise)
...
ggplot(spotify2, aes(x=is_popular, y=danceability)) + geom_boxplot()

#boxplot popular vs energy
ggplot(spotify2, aes(x=is_popular, y=energy)) + geom_boxplot()

#boxplot popular vs valence, not much difference
ggplot(spotify2, aes(x=is_popular, y=valence)) + geom_boxplot()

#more prevalent mode split by popular and not
ggplot(spotify2, aes(x=is_popular, fill=mode)) + geom_bar(position = position_dodge()) 
